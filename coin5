...
# Define a function to check for take profit
def check_take_profit(current_price):
    if current_price >= take_profit:
        print("Take profit reached at", current_price)
        # Close the trade
        return True
    else:
        return False

# Define a function to print out trade information
def print_trade_info(current_price):
...
while True:
    ...
    current_price = 95.0  # this should be replaced with a live price
    print_trade_info(current_price)
    if check_take_profit(current_price):
        break
